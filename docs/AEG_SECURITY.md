...
ID, version, kind, capabilities and runtime status. Modules may observe the
shared redacted event stream and contribute a stricter Effect decision. A
...
Built-in modules cover identity and delegation, Runtime containment,
filesystem effects, external HTTP, approval, trace correlation and the audit
ledger. Additional content, MCP, database, memory or provider guards can use
...
```text
Stage → Execute → Collect file + HTTP effects → Authorize → Approve → Commit / Execute / Rollback
```
...
4. A deterministic built-in policy evaluates every effect. The most restrictive
...
The Security Center polls the overview projection and displays six correlated
lanes: Identity, Runtime, Policy, Approval, Effect, and Evidence. The Playground
keeps only action-time warnings and links to the complete operator view.
